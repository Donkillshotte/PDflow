...
export async function POST(req: Request) {
  const denied = authorizeStudioMutation(req, "preview analysis bundle");
  if (denied) return denied;
  const tooLarge = rejectOversizedBody(req, 64 * 1024);
  if (tooLarge) return tooLarge;
  const payload = await req.json();
  const remote = await agentResponse("/v1/analysis-bundles/preview", {
    method: "POST",
  ...
  if (!remote) {
    return NextResponse.json(
      { status: "GAP", ready: false, reason: "PDflow local agent is not running" },
      { status: 503 },
    );
  }
