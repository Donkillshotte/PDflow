...
- Extraction and EM live in Python (`pdn_extract`, `pdn_em`): tech LEF WIDTH/THICKNESS/RPERSQ; strap width from \(w=\mathrm{RPERSQ}\cdot L/R\), not min WIDTH. SPEF PG C is stamped only from a power `*D_NET` by name-join onto `write_pg_spice` nodes (GCD OpenRCX SPEF has no VDD). Overlapping-strap Cox is ε0 εr (SiO2) lateral \(t L_\mathrm{ov}/d_\mathrm{gap}\) plus adjacent-layer plate \(A_\mathrm{ov}/h_\mathrm{ILD}\); not LEF CPERSQDIST, not foundry PEX, not GCD default.
