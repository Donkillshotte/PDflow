...
def _check_enterprise_docs(check, root: Path) -> None:
    ...
    course_md = (root / "docs/course.md").read_text()
    check("/api/story" in course_md, "course docs name /api/story")
    check("leftover-free" in course_md and "resp_msg[14]" in course_md, "course docs name leftover vs leftover-free")
    home = (root / "studio/src/app/page.tsx").read_text()
    check("ProductStory" in home, "home mounts ProductStory")
    check("getProductStory" in home, "home server-renders the story snapshot")
    check('"use client"' not in home, "home page is a server component")
    check("Interactive flow" not in home, "home does not use brochure Interactive flow")
    check("Course per lesson" not in home, "home does not use a generic lesson UX strip")
    check("STA · DRC · LVS · power" in home, "home finish step names the four pillars")
    check("leftover setup stays" in home.lower() and "resp_msg[14]" in home, "home rail names leftover setup vs eco I/O")
    check("Leftover stays named" in (root / "studio/src/app/home-hero.tsx").read_text(), "home hero names leftover")
    css = (root / "studio/src/app/globals.css").read_text()
    hero_block = css.split(".hero {")[1].split(".hero-metal")[0]
    check("100vh" not in hero_block, "home hero is not a full-viewport brochure")
    story_ui = (root / "studio/src/components/ProductStory.tsx").read_text()
    check('data-id={step.id}' in story_ui, "story leftover steps can span for readable leftover text")
    ...
    studio_readme = (root / "studio/README.md").read_text()
    check("/tools#suite" in studio_readme, "studio README sends the hook matrix to /tools#suite")
    check("/product" in studio_readme, "studio README lists /product")
    check("Leftover stays named" in studio_readme, "studio README names leftover on signoff")
    check("resp_msg[14]" in studio_readme and "R2R MET" in studio_readme, "studio README names the eco_scratch I/O leftover")
    check("initial" in story_ui, "ProductStory can take a server snapshot")
    ...
    check((root / "learn/scripts/validate_lab_physics.py").is_file(), "validate_lab_physics.py exists")
    check((root / "studio/src/lib/lab.ts").is_file(), "studio lab snapshot exists")
    check((root / "studio/src/app/lab/page.tsx").is_file(), "studio /lab page exists")
    check((root / "studio/src/app/product/page.tsx").is_file(), "studio /product page exists")
    check((root / "studio/src/components/LabBench.tsx").is_file(), "LabBench exists")
    check('href: "/product"' in story_txt, "story Product surface points at /product")
    check("/product" in (root / "studio/src/components/SiteNav.tsx").read_text(), "nav lists Product")
    check("/product" in (root / "studio/src/components/LabBench.tsx").read_text(), "LabBench sends wins to /product")
    check("Last cook vs slot base" not in (root / "studio/src/components/LabBench.tsx").read_text(), "LabBench does not host the win_rule table")
    check("Δleak" in (root / "studio/src/app/product/page.tsx").read_text(), "product page reports leakage with area/power/IR")
    check((root / "learn/scripts/record_dse_launch.py").is_file(), "record_dse_launch.py exists")
    check("record_dse_launch.py" in (root / "learn/scripts/run_dse.sh").read_text(), "run_dse.sh stamps a launch shot")
    ...
    check("hands-on" not in intro.lower() and "black magic" not in intro.lower(), "lesson 00 intro is technical")
    ...
    check((root / "learn/scripts/lab_asap7_mmmc.py").is_file(), "ASAP7 MMMC pair script exists")
    script_md = (root / "docs/script.md").read_text()
    check("lab_asap7_lvs.py" in script_md, "script.md lists leftover-named ASAP7 LVS")
    check("lab_asap7_mmmc.py" in script_md, "script.md lists ASAP7 MMMC pair")
    check("lab_asap7_spice.py" in script_md, "script.md lists leftover-named ASAP7 Xyce")
    check("run_lab_asap7_pdk.sh" in script_md, "script.md lists ASAP7 layer-1 wrapper")
    ...
    check("asap7_cook" in (root / "studio/src/lib/suite.ts").read_text(), "suite has ASAP7 cook hook")
    check("asap7_drc" in (root / "studio/src/lib/suite.ts").read_text(), "suite has ASAP7 DRC hook")
    check("asap7_lvs" in (root / "studio/src/lib/suite.ts").read_text(), "suite has ASAP7 LVS hook")
    check("asap7_mmmc" in (root / "studio/src/lib/suite.ts").read_text(), "suite has ASAP7 MMMC hook")
    check("asap7_pkg" in (root / "studio/src/lib/suite.ts").read_text(), "suite has leftover-named ASAP7 PKG hook")
    check("asap7_chip_pdn" in (root / "studio/src/lib/suite.ts").read_text(), "suite has leftover-named ASAP7 chip PDN hook")
    check("run_asap7_e2e.py" in (root / "docs/script.md").read_text(), "script.md lists ASAP7 e2e runner")
    check("lab_asap7_drc.py" in (root / "docs/script.md").read_text(), "script.md lists leftover-named ASAP7 DRC")
    check("lab_asap7_pkg.py" in script_md, "script.md lists leftover-named ASAP7 PKG")
    check("lab_asap7_chip_pdn.py" in script_md, "script.md lists leftover-named ASAP7 chip PDN")
    check((root / "learn/scripts/run_asap7_e2e.py").is_file(), "ASAP7 e2e runner exists")
    check((root / "learn/scripts/lab_asap7_drc.py").is_file(), "leftover-named ASAP7 DRC script exists")
    check("ASAP7 (Lab)" in (root / "learn/reference/suite-status.md").read_text(), "suite-status has ASAP7 section")
    check("lab_asap7_pdk" in (root / "studio/src/lib/run.ts").read_text(), "run.ts allows lab_asap7_pdk")
    check("lab_asap7_pkg" in (root / "studio/src/lib/run.ts").read_text(), "run.ts allows leftover-named lab_asap7_pkg")
    check("lab_asap7_chip_pdn" in (root / "studio/src/lib/run.ts").read_text(), "run.ts allows leftover-named lab_asap7_chip_pdn")
    check("asap7_calibre_gated" in (root / "learn/signoff/leftover_catalog.json").read_text(), "catalog names leftover ASAP7 Calibre")
    check("asap7_dummy_bump" in (root / "learn/signoff/leftover_catalog.json").read_text(), "catalog names leftover ASAP7 dummy bump")
    check("asap7_sidecar_rdl" in (root / "learn/signoff/leftover_catalog.json").read_text(), "catalog names leftover ASAP7 sidecar RDL")
    check("asap7_compact_pkg" in (root / "learn/signoff/leftover_catalog.json").read_text(), "catalog names leftover ASAP7 compact PKG")
    check((root / "learn/scripts/lab_asap7_pkg.py").is_file(), "leftover-named ASAP7 PKG script exists")
    check((root / "learn/scripts/run_lab_asap7_pkg.sh").is_file(), "ASAP7 leftover PKG wrapper exists")
    ...
    check("asap7_chip_mesh" in (root / "learn/signoff/leftover_catalog.json").read_text(), "catalog names leftover ASAP7 chip mesh")
    check("Dummy bump" in bench, "LabBench shows leftover ASAP7 dummy bump")
    check("Sidecar RDL" in bench, "LabBench shows leftover ASAP7 sidecar RDL")
    check("Compact droop" in bench, "LabBench shows leftover ASAP7 compact droop")
    check("Chip mesh static" in bench, "LabBench shows leftover ASAP7 chip mesh static")
    check("Chip mesh transient" in bench, "LabBench shows leftover ASAP7 chip mesh transient")
    check("fmaxGhz" in bench or "fmax / period_min" in bench, "LabBench shows ASAP7 fmax")
    check("periodMinPs" in bench or "period_min" in bench, "LabBench shows ASAP7 period_min")
    check("cookCount" in bench, "LabBench shows ASAP7 folio cook count")
    check("45.298" in bench, "LabBench still names Nangate 45.298 as not comparable")
    ...
    check("signoff_all_eco_scratch.json" in story_txt, "story ECO reads leftover setup from the copy close")
    check("leftoverSetupOpenDetail" in story_txt, "story ECO names I/O leftover vs R2R MET")
