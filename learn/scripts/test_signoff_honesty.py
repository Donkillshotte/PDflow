...
def main() -> int:
    ...
    check(
        "RTL → finish" in flowlab and "open items stay listed on signoff" in flowlab,
    ...
    check('id="ir"' in heatmap, "heatmap owns the #ir deep link")
    check("<DynamicIrHeatmap" in finish.split("Individual STA")[0], "heatmap sits with STA IR-aware on finish")
    panel = (ROOT / "studio/src/components/flowlab/StaIrAwarePanel.tsx").read_text()
    check("sta-ir-gates" in panel, "per-gate IR overlay is behind a details, not a wall of rows")
    ...
    strip = (ROOT / "studio/src/components/LeftoverSuiteStrip.tsx").read_text()
    check('fetch("/api/suite")' in strip, "leftover suite strip loads live leftover ids")
    check("leftoverNamedIds" in strip, "leftover suite strip filters built leftover chips")
    check("leftover-free" not in strip, "leftover suite strip does not claim leftover-free")
    tools_ui = (ROOT / "studio/src/app/tools/tools-client.tsx").read_text()
    check("SurfaceRail" in tools_ui, "tools page shows the three-surface rail")
    check("LeftoverSuiteStrip" in tools_ui, "tools banner names leftover from the suite")
    materials_ui = (ROOT / "studio/src/app/materials/page.tsx").read_text()
    check("SurfaceRail" in materials_ui, "materials page shows the three-surface rail")
    check("LeftoverSuiteStrip" in materials_ui, "materials names leftover on finish/suite")
    check("finish signoff matrix" in materials_ui, "materials links leftover to the finish matrix")
    lesson_ui = (ROOT / "studio/src/app/lessons/[id]/page.tsx").read_text()
    check("SurfaceRail" in lesson_ui, "lesson detail shows the three-surface rail")
    check("LeftoverSuiteStrip" in lesson_ui, "lesson detail names leftover on finish")
    check("0/8" in lesson_ui, "lesson detail names leftover student pace")
    check("LeftoverSuiteStrip" in flowlab, "FlowLab hero names leftover from the suite")
    css = (ROOT / "studio/src/app/globals.css").read_text()
    check(".fl-pro .leftover-chip" in css, "FlowLab leftover chips have a dark theme")
    open_src = (ROOT / "studio/src/lib/open.ts").read_text()
    check('id: "dash-leftover"' in open_src, "command palette has leftover named")
    check("Finish · leftover named" in open_src, "command palette leftover opens finish signoff")
    check("Suite leftover named" in open_src, "command palette leftover opens the suite")
    check("Lab surface" in open_src and "Product surface" in open_src, "command palette jumps Course/Lab/Product")
    nav = (ROOT / "studio/src/components/SiteNav.tsx").read_text()
    check("nav-kbd-hint" in nav, "nav shows Ctrl+K for the command palette")
