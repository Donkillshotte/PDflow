...
def main() -> int:
    ...
    if live.is_file():
        ...
        sta_signoff = json.loads((ROOT / "learn/sim/reports/sta_signoff_flowlab.json").read_text())
        wns = float((sta_signoff.get("timing") or {}).get("wns_ns"))
        slack = float(blob["sta"]["slack_ns"])
        slack_ir = float(blob["sta"]["slack_ir_ns"])
        check(abs(slack - wns) < 0.02, "live IR-aware path slack matches sta_signoff WNS")
        check(slack < 0, "live IR-aware path is the violating WNS path, not ideal-RC MET")
        check(slack_ir <= slack + 1e-9, "IR overlay does not improve WNS")
        n_gates = int(blob["sta"]["n_gates"] or 0)
        n_joined = int(blob["sta"]["n_joined"] or 0)
        check(n_gates >= 1 and n_joined == n_gates, "live path gates are ITerm-joined")
        check(len(blob.get("path_gates") or []) == n_gates, "live path_gates")
