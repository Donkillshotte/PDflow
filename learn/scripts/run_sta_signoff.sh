#!/usr/bin/env bash
# STA signoff: OpenSTA on 6_final.v + SPEF + SDC from this invocation
# Env: FLOW_VARIANT=learn|flowlab
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
if ! "${ROOT}/scripts/resource_guard.sh"; then
  exec "${ROOT}/scripts/run_resource_job.sh" sta-signoff bash "${BASH_SOURCE[0]}" "$@"
fi
...
# PD_FLOW_SDC_FILE may point at the exact current-run constraint artifact
# generated by ORFS. Falling back to the course constraint preserves the
# legacy CLI.
...
# OpenSTA emits a trailing space in its startup banner on some builds. Keep
# generated current-run logs deterministic so the repository diff stays clean.
sed -i 's/[[:space:]]\+$//' "${STA_LOG}"
...
timing_eval = evald.get("pillars", {}).get("timing", {})
requirement_status = str(
    timing_eval.get("status")
    or ("PASS" if timing_eval.get("ok") is True else "GAP")
)
evidence_status = "PASS" if timing_eval.get("evidence_ok") is True else "GAP"
out = {
  "kind": "sta_signoff",
  "variant": "${VARIANT}",
  "engine": "opensta",
  ...
  "signoff_status": "PASS" if timing_eval.get("ok") is True else requirement_status,
  "product_signoff": bool(timing_eval.get("ok") is True),
  "timing": metrics["timing"],
  "evaluation": evald.get("pillars", {}).get("timing", {}),
  "ok": timing_eval.get("ok"),
...
if ir_p.is_file():
    ...
    blob["ir_aware"] = {
        ...
        "note": "current-run NLDM × ITerm V; does not change nominal WNS",
    }
