...
def main() -> int:
    ...
    check("-skip_buffering" in tcl and "sizeup,swap" in tcl, "size-up pass skips BufferMove while SPEF is loaded")
    check("estimate_parasitics -global_routing" in tcl, "buffer pass uses GRT parasitics")
    check('-sequence "buffer"' in tcl, "second setup pass is BufferMove")
    check("ECO_REPAIR_BUFFER" in tcl, "repair tcl stamps a buffer pass")
    check("ECO_PHASE" in tcl and "buffer" in tcl, "repair tcl has a buffer phase")
    check("ECO_PHASE" in tcl and '"io"' in tcl, "repair tcl has an I/O size-up phase")
    check("ECO_REPAIR_IO" in tcl, "repair tcl stamps I/O size-up")
    check("set_false_path -from [all_registers] -to [all_registers]" in tcl, "I/O phase false-paths R2R so size-up hits outputs")
    ...
    check('phase="io"' in src, "apply launches a third OpenROAD for leftover I/O")
    check("ECO_KEEP_SIZEUP" in src, "apply keeps the SPEF size-up if BufferMove cannot legalize")
    check("ECO_KEEP_BUFFER" in src, "apply keeps the buffered netlist if I/O size-up cannot legalize")
    ...
    r2r_size = tcl.find('-sequence "sizeup,swap"', focus_cmd)
    check(focus_cmd >= 0 and r2r_size > focus_cmd, "I/O false path is set before R2R size-up")
    ...
    check("legenda" not in canvas and "correlati" not in canvas, "layout canvas legend is English")
    param = (ROOT / "studio/src/components/flowlab/FlowLabParamStudio.tsx").read_text()
    check("periodo clock" not in param and "facile" not in param, "param studio SDC is English")
    check("mappatura" not in param and "Minimizza" not in param, "param studio ABC is English")
    check("locked?: boolean" in param, "param studio can lock knobs after finish")
    check("gcd/flowlab" in param, "param studio names the locked path")
    term = (ROOT / "studio/src/components/flowlab/FlowLabTerminal.tsx").read_text()
    check("«Run phase»" not in term and "Run phase to start" in term, "terminal empty hint is English")
    check("FLOWLAB_LOCKED_RECOOK" in (ROOT / "studio/src/lib/actions.ts").read_text(), "ORFS recook set is named")
    jobs = (ROOT / "studio/src/lib/jobs.ts").read_text()
    check("FLOWLAB_LOCKED_RECOOK.has(action)" in jobs, "preflight refuses locked flowlab recook")
    check("overwrite gcd/flowlab" in jobs, "recook refuse names the locked path")
    check("«" not in flow and "»" not in flow, "FlowLab copy has no guillemets")
    check("recookBlocked" in flow, "FlowLab blocks ORFS recook after finish")
    ...
    check("DRT-0206" in panel, "EcoPanel names the DRT leftover")
    ...
    if live.is_file():
        ...
        if scratch.get("ok"):
            ...
            if close.is_file():
                ...
                wns = float(setup["wns_ns"]) if setup.get("wns_ns") is not None else None
                summary = str(sig.get("summary") or "")
                if setup.get("setup_open"):
                    check(wns is not None and wns < 0, "eco close setup leftover WNS is negative")
                    check("leftover setup open" in summary, "eco close summary names leftover setup")
                else:
                    check("leftover setup open" not in summary, "MET setup does not keep leftover setup open")
