...
ADAPTERS: dict[str, dict] = {
    ...
    "cell": {
        ...
        "note": "attributed STA path drive-up (module-scoped), I-scale-win IR-hotspot ODB-join, I-scale-champ ODB-join + write_pg_spice residual vs IR-cell extract (re-paid per winning_ir extract), leftover champ-extract cone size-up + write_pg_spice residual vs IR-cell-champ extract, seq-heavy leftover-cone region density cap (not more combo size-up), seq-heavy winning-IR 1× rXY density cap on the IR-cell netlist (not leftover-cone rXY; re-paid when the residual hotspot leaves the capped bin, not IR-cell-region rXY), leftover combo cells on the winning-IR-region PDN join (minus IR-cell / champ / leftover-cone) on the IR-cell netlist, leftover leftover cells on that leftover-combo PDN join (minus leftover-combo / IR-cell / champ / leftover-cone) on the leftover-combo netlist, and leftover leftover leftover cells on that leftover leftover PDN join (minus leftover leftover / leftover-combo / IR-cell / champ / leftover-cone) on the leftover leftover netlist; not ABC ops, not a chip restart",
    },
    "net": {
