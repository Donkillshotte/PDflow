...
ADAPTERS: dict[str, dict] = {
    ...
    "cell": {
        ...
        "note": "attributed STA path drive-up (module-scoped), I-scale-win IR-hotspot ODB-join, I-scale-champ ODB-join + write_pg_spice residual vs IR-cell extract (re-paid per winning_ir extract), leftover champ-extract cone size-up + write_pg_spice residual vs IR-cell-champ extract, seq-heavy leftover-cone region density cap (not more combo size-up), and seq-heavy winning-IR 1× rXY density cap on the IR-cell netlist (not leftover-cone rXY; re-paid when the residual hotspot leaves the capped bin, not IR-cell-region rXY); not ABC ops, not a chip restart",
    },
    "net": {
