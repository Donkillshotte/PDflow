...
- Draw die/core/rows su carta partendo dal log
...
Read: `learn/reference/walkthrough-floorplan.tcl.md` (30 min consigliati).

---

## Part 2 — Esecuzione floorplan (20 min)
...
**Exercise B1 workbook:** ripeti con `CORE_UTILIZATION=25` e `50`.

```bash
cd tools/OpenROAD-flow-scripts/flow
make DESIGN_CONFIG=./designs/nangate45/gcd-tutorial/config.mk \
     FLOW_VARIANT=learn CORE_UTILIZATION=25 clean_floorplan floorplan
# note Core area dal log 2_1_floorplan.log
make DESIGN_CONFIG=./designs/nangate45/gcd-tutorial/config.mk \
     FLOW_VARIANT=learn CORE_UTILIZATION=50 clean_floorplan floorplan
```

Question: core area scale linearly with 1/utilization? (approximately yes)
...
Draw a mano: M1 followpin → M4 → M7
...
Checklist:
- [ ] Fit (`F`) — two rectangles concentrici (die / core)
...
Checklist (colors Nangate45 in *questa* GUI):
- [ ] Linee blu fitte = rail M1 followpin
- [ ] Strap verdi verticali + rosa orizzontali
- [ ] Display Control: spegni metal2/metal3 per “pulire” il segnale (non c’is ancora)
- [ ] Tcl: `gui::set_display_controls "Nets/Power" visible true`
- [ ] Tapcell: `gui_2_3_floorplan_tapcell.odb` or PNG `win_tapcell.png`

**Scavenger hunt B3:** note nel notebook i colors strap vs rail; compare with `03_pdn_labeled.png`.

---

## Part 6 — Confronto pre/post floorplan (10 min)

| Files | Celle posizionate? | Routing? | PDN? |
|---|---|---|---|
...
- [ ] Table utilization vs core area (3 righe)
- [ ] PDN spiegato aloud in 60 secondi
