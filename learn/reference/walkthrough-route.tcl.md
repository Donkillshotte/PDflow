# Annotated walkthrough — global_route.tcl and detail_route.tcl
...
Produces **guides**, not mask wire. Failure: `5_1_grt-failed.odb` + congestion report. GUI: `gui_grt` as the `detail_route.tcl` error message says.
...
Teaching note: GRT is not one-shot. It is a loop with RSZ. For this the log is long and area rises again (remember CTS 48% → finish still has fill).
...
Then `set_propagated_clock`, TritonRoute arguments (`VIA_IN_PIN_*`, `OR_SEED`, `DETAILED_ROUTE_END_ITERATION`).

Default policy (comment lines 30–42): if you do not set `DETAILED_ROUTE_ARGS`, use `-drc_report_iter_step 5` so it does not flood you with huge DRC at iter 1–2.
...
Then `check_antennas -report_file drt_antennas.log`.  
...
1. Why does DRT require GRT?
2. Empty `5_route_drc.rpt` = ?
3. Why `estimate_parasitics -global_routing` > `-placement`?
4. What does `-droute_end_iter 5` do?
