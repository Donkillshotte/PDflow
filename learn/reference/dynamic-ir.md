...
Frontend: OpenROAD `write_pg_spice`. **Solver A** (BE + LU + \(i_L\)) is l’oracle. **Solver B** (SA-AMG + CG) is the workhorse on the same \(A=G+C/\Delta t+g_\mathrm{eq}\). **Solver C** is MOR: RC su \(\delta v\), o descriptor RLC su \(x=[v;i_L]\) (stessa fisica del companion). **Solver D** is RAS Schwarz (graph partition, local LU, GMRES). vyges-em-ir is bootstrap.
...
pdn_dynamic.py
    ...
    C_rr opt-in (`--rail-c`) su instance pin; Cox strap opt-in (`--rail-c-geom`,
...
vyges-em-ir today is essentially **L1 simultaneousus** (all the cells a `switch_t_ns`). Qui:

| Livello | Idea | GCD status |
|---|---|---|
| **L0 Static** | \(G V = I_\mathrm{avg}\) | READY — same mesh di PDNSim |
...
| **L2 VCD/SAIF dynamic** | tempi reali / idle | **GAP** on RTL VCD (`tb_gcd` ≠ ITerm); name-join READY on synthetic VCD/SAIF in tests. SAIF does not invent t50 and does not rescale \(I_\mathrm{avg}\) da TC; TC=0 azzera l’impulso (idle). FSDB = GAP (binario proprietario) |
...
L’interpolatore CCS esiste (`pdn_current.py`) e is tested on synthetic Liberty — **does not** invent NLDM→CCS mapping.
...
| **D** RAS Schwarz | domain decomposition su \(A\) e su \(K\) descriptor | **READY** companion GCD · ndom=8 · 45.284 mV · \|A−D\| **0.013 mV**; kind=2 su \(K\) unsymmetric (32-nodo R+L, not the default GCD) |
...
| Dual-rail VSS | return path, same \(I(t)\) | **READY** extract+TRAN: `write_pg_spice -net VSS`, pair `* Sink for inst/pin`, bounce = −Vmin; **does not** change gold VDD 45.298 mV; C rail-to-rail **opt-in**: instance pin (`--rail-c` / `RAIL_C=1`, scenario F) e/o strap Cox (`--rail-c-geom` / `RAIL_C_GEOM=1`, ε0εr lateral + ILD plate, not PEX foundry). GCD extract: **6591** lateral pairs, **0** plate (PDN almost all metal1; metal4 is not adjacent), \(C_\Sigma=3.37\,\mathrm{fF}\) — not in default TRAN |
...
| 4 | Current waveform | triangle per ITerm | CCS lagged \(I(\mathrm{slew},V^n)\) o ECSM \(\|C\mathrm{d}V/\mathrm{d}t\|\) if tables + slew/c_load; Nangate = GAP |
| 5 | Transient solver | **A** LU gold + **B** SA-AMG + **C** descriptor RLC Krylov + **D** RAS (companion GCD; kind=2 su \(K\) unsymmetric) + **N4** descriptor BE nativo (sparse \(E\), \(n_\mathrm{iv}\)) + kind=3 BiCGSTAB workhorse + adaptive Δt on descriptor + MOR gen sparse-\(E\) (opt-in on-die L, not GCD gold) + VSS return TRAN + opt-in coupled \(C_{rr}\) / strap Cox (sparse \(C\), native `hist_cmat`) | ngspice = gold 1-node RC, R+L, VRM+die, strap K, 1-node thermal analogue, **2-nodo \(C_{rr}\)/Cox**; Xyce = GAP in VM (deck contract); Native int64 index; Ginkgo GPU = GAP |
| 6 | Analysis | heatmap, windows, ranking, path STA delay, \(J=I/(wt)\), **thermal mesh** strap+via+ILD/Si lumpato + **native thermal BE** (max \(\Delta T\)) + **R(T) TRAN** one-shot (Solver A, not gold) | relative TTF (no foundry A); 3D FEM/package CFD = GAP; skin δ reported not printed in G; path = NLDM typical-V; N1 restamp + TRAN weakly-coupled (Si excluded from metric \(\Delta T\)); gold TRAN remains **45.298 mV** |
...
This is not sign-off Ansys RedHawk / Cadence Voltus. Nangate45 does not have CCS current tables. RTL VCD (`tb_gcd`, 10 ns) **does not** name gate pins of the 0.46 ns netlist — no fake mapping. t50 clock = OpenSTA `report_arrival` (join on instance name). Dump STA di default **without SPEF** (ideal interconnect; `STA_SPEF` for OpenRCX). Path delay scala only gates NLDM typical-V, not a liberty at Vmin.

## Modi I(t)

For every load ITerm: \(I_\mathrm{leak}=f_\mathrm{leak}\,I_\mathrm{avg}\), triangular impulse of duration `DUR_NS` con carica about \((I_\mathrm{avg}-I_\mathrm{leak})\,T_\mathrm{clk}\), clip a `PEAK_FACTOR·I_avg`.
...
| Dual-rail VSS return | — | **26.707 mV bounce** @ 0.21 ns · 601/601 Sink-for pairs · 3381 nodi / 12 pad (mesh VSS, non VDD) · native_hist. **Non** is the gold VDD 45.298 mV |
...
Con \(i_L\), the simultaneousus spike is the worst (I_peak 52 mA vs 11 mA clock STA).
The previous synthetic clock (59.925 mV) did not join ITerms (raggio 800 dbu < offset pin VDD ~1.2 µm).

Gold ngspice: **1 nodo RC** `|V_BE−V_ng| ≈ 0.032 mV`; **1 nodo pad–R–L–C** ≈ 0.056 mV (`gear maxord=1`, soglia 5 mV). This is not the chip.

EM: \(I=(V_a-V_b)/R\) e \(J=I/(w t)\) con \(w=\max(\mathrm{RPERSQ}\cdot L/R,\,\mathrm{WIDTH}_\min)\) from tech LEF. TTF relativo \((J_\mathrm{ref}/J)^n\), \(n=2\), \(J_\mathrm{ref}=10^{10}\,\mathrm{A/m^2}\) — **not** foundry hours. \(\Delta T\) **metal-graph** \(G_\mathrm{th}=k_{\mathrm{Cu}}A/L\) on strap **and** adjacent vias (LEF `HEIGHT`/`CUT`) **plus** ILD \(G_\mathrm{ild}=k_{\mathrm{ox}}(w L)/\mathrm{HEIGHT}\) toward a lumped Si node \(G_\mathrm{vert}=k_{\mathrm{Si}}A_\mathrm{die}/t_\mathrm{wafer}\) (\(t=300\,\mu\mathrm{m}\), not GDS) that stars on pads. Without HEIGHT and without via stack remains GAP. Lumped \(R_{\mathrm{th}}I^2R\) remains comparison. This is not an FEM 3D / CFD package. Skin \(\delta\) reported; metal1 Nangate \(t\ll\delta\) at clock GCD \(\Rightarrow R_\mathrm{ac}/R_\mathrm{dc}\approx 1\) (non stampato in \(G\)).

GCD clock STA: \|I\|_max ≈ 2.25 mA (via / strap). \(J_\max\) ≈ \(1.48\times10^{11}\,\mathrm{A/m^2}\) on metal1 (w clamped to 0.07 µm; \(I\) ≈ 1.35 mA) · TTF_rel ≈ \(4.56\times10^{-3}\) · \(\Delta T\) lumpato ≈ 11 mK. Thermal mesh on TRAN \(t_\mathrm{worst}\): 5153 strap + 39 via + **ILD→Si** (5153 \(G_\mathrm{ild}\), 1 wafer node \(t=300\,\mu\mathrm{m}\)) + 13 pad · \(\Delta T_\mathrm{mesh}\approx 0.66\,\mathrm{K}\) (metal) · \(\Delta T_\mathrm{Si}\approx 0.057\,\mathrm{K}\) · pad \(\approx 0.028\,\mathrm{K}\) · N1 restamp \(\Delta\)IR \(\approx 0.019\,\mathrm{mV}\) · **R(T) TRAN 45.311 mV (\(\Delta\) +0.013 mV)** one-shot weakly coupled (\(T\) stazionario da \(I_\mathrm{avg}\), poi Solver A su \(R(T)\); **non** entra nel gold TRAN **45.298 mV**). Thermal tau ≫ \(\Delta t\) electric — is not electro-thermal DAE at step. Without ILD/Si far-node metal-only was ~320 K — wrong path, not gold. Skin \(\delta\approx 1.4\,\mu\mathrm{m}\), \(R_\mathrm{ac}/R_\mathrm{dc}=1\). \(i_L\) bump max ≈ 1.67 mA. Path STA: OpenSTA worst max path, delay gate \(\times(V_\mathrm{dd}/V_\mathrm{inst})^{1.3}\) (NLDM typical-V, non una seconda liberty a Vmin). L3 prefix BE 38/74 step, \|A−W\|=0 (L/R ≈ 4 ns ≫ orizzonte 0.74 ns — no isolated restart).
...
| `learn/scripts/pdn_em.py` | J da RPERSQ·L/R, TTF relativo, thermal mesh strap+via+ILD/Si, skin δ, restamp R(T) N1 |
| `learn/scripts/pdn_dynamic.py` `electrothermal_timestep_be` | one-shot Solver A TRAN su \(R(T)\) (not gold) |
| `learn/scripts/pdn_current.py` | triangle + interpolatori CCS e ECSM (never from NLDM) |
| `learn/scripts/pdn_activity.py` | synthetic t50 + STA `report_arrival` + VCD/SAIF name-join + finestre I_tot |
...
Classroom limits: triangle ≠ CCS; AMG on the GCD is slower than LU (4k nodi); package R/L lumpato; pad PDNSim su metal4.
