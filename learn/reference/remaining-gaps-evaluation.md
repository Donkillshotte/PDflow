...
| **ECO after finish** | **Closed as propose/apply + signoff_all on a copy** | Done | Apply refused on flowlab. Live: SPEF size-up, BufferMove without SPEF (29 buffers), `global_connect`. eco_scratch OpenSTA: R2R MET (~3 ps), leftover is course 20% output delay (WNS −0.01 on `resp_msg[14]`). Shared NAND2_X2 `_647_` also drives R2R `_809_`. I/O `repair_timing`, BUF_X4 on `output42`, RSZ `clone,split`, and a manual `_647_` clone all regress R2R — not applied. Leftover is SDC-gated. Close is `signoff_all`. |
