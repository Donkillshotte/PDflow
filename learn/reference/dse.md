...
RTL
 ...
 → F4 static-IR-steer: `winning_static_pdn` (ranking 1× a parte) paga `pkg_r` — decap/pkg L non muovono il DC, non Dynamic IR-steer, non gold
 → F4 static-mesh: residuale pkg_r nullo (bump V ideali) paga bump più densi sullo stesso ODB — non un nuovo GPL, non gold
...
| **pdn** | `c_decap`, pkg L, **pkg_r (static IR)**, **bump pitch (on-die static)**, I(t)×power, mesh candidato/host, solver MF | ingest gold + extract candidato + **extract host** + **extract host-regione** + DirectLU/AMG/RAS/Krylov + **AMG/RAS/Krylov su winning_ir_pdn** + **pkg_r su winning_static_pdn** + **bump restamp sullo stesso ODB** (non gold) |
