...
| ECO after finish | Built: Propose on locked variants. Apply on an unlocked copy is two OpenROAD processes: SPEF size-up (`sizeup,swap`), then BufferMove with no SPEF (`estimate_parasitics -global_routing` before incremental GRT). SPEF + BufferMove in one session is RSZ-0074. `global_connect` after repair so new cells have VDD/VSS. Size-up false-paths outputs so RSZ hits register-to-register. Live: 29 buffers, R2R MET, leftover WNS −0.01 is course 20% output delay on `resp_msg[14]`. Repairing that shared cone (or upsizing the BUF_X1 output driver to BUF_X4) regresses R2R. Leftover is SDC-gated, not a missing ECO step. If TritonRoute cannot connect, apply keeps the last legal netlist. Never calls `signoff_all`. |
