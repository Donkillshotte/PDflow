...
| ECO after finish | Built: Propose on locked variants. Apply on an unlocked copy is two OpenROAD processes: SPEF size-up (`sizeup,swap`), then BufferMove with no SPEF (`estimate_parasitics -global_routing` before incremental GRT). SPEF + BufferMove in one session is RSZ-0074. `global_connect` after repair so new cells have VDD/VSS. Size-up false-paths outputs so RSZ hits register-to-register. Live: 29 buffers, R2R MET (~3 ps), leftover WNS −0.01 is course 20% output delay on `resp_msg[14]`. That pin shares NAND2_X2 `_647_` with R2R NAND2_X4 `_809_`. Repairing the shared cone, BUF_X1→BUF_X4 on `output42`, RSZ `clone,split` (cloned a NAND4, not `_647_`), and a manual clone of `_647_` all regress R2R. I/O-only liberty swaps of `output42` to CLKBUF_X1/X2/X3 or BUF_X2 stay at WNS −0.01 (last-stage delay is not the 10 ps). Not a third apply phase. Leftover is SDC-gated. If TritonRoute cannot connect, apply keeps the last legal netlist. Never calls `signoff_all`. |
