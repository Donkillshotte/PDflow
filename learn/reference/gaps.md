...
| ECO after finish | Built: Propose on locked variants. Apply on an unlocked copy is three OpenROAD processes: SPEF size-up, BufferMove with no SPEF, then SPEF I/O size-up (`ECO_PHASE=io`, R2R false so leftover `resp_msg` is repaired). SPEF + BufferMove in one session is RSZ-0074. `global_connect` after repair so new cells have VDD/VSS. Size-up false-paths outputs; I/O phase false-paths register-to-register. If TritonRoute cannot connect, apply keeps the last legal netlist. A legal repair may still leave setup open; leftover is named. Never calls `signoff_all`. |
