...
| ECO after finish | Propose on locked variants. Apply on an unlocked copy loads SPEF before size-only `repair_timing` (BufferMove SIGSEGVs / RSZ-0074 on this ODB). Size-up then fails TritonRoute `checkConnectivity`; apply restores the source `6_final` instead of writing a broken netlist. Never calls `signoff_all`. |
