...
| ECO after finish | Built: Propose on locked variants. Apply on an unlocked copy is two OpenROAD processes: SPEF size-up (`sizeup,swap`), then BufferMove with no SPEF (`estimate_parasitics -global_routing` before incremental GRT). SPEF + BufferMove in one session is RSZ-0074 even after `estimate_parasitics`. Size-up sets a session `set_false_path -to [all_outputs]` so OpenROAD does not spend the budget on `resp_msg[*]`. If TritonRoute cannot connect (DRT-0206), apply restores the source `6_final` (size-up) or keeps the size-up if only BufferMove fails. A legal repair may still leave setup open; leftover is named. Never calls `signoff_all`. |
