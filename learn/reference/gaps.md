...
| ECO after finish | Built: Propose on locked variants. Apply on an unlocked copy loads SPEF, wraps DPL in incremental GRT, then detailed_route (`-clean_patches`). Size-up uses SPEF (`sizeup,swap`). BufferMove on SPEF hits RSZ-0074; a second pass uses `estimate_parasitics -global_routing` and a bounded BufferMove, caught if RSZ-0074 returns. Size-up sets a session `set_false_path -to [all_outputs]` so OpenROAD does not spend the budget on `resp_msg[*]`. If TritonRoute cannot connect (DRT-0206), apply restores the source `6_final`. A legal repair may still leave setup open; leftover is named. Never calls `signoff_all`. |
