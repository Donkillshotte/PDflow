...
| ECO after finish | Built: Propose on locked variants. Apply on an unlocked copy loads SPEF, wraps DPL in incremental GRT, then detailed_route (`-clean_patches`). BufferMove is unsafe (SIGSEGV / RSZ-0074). Size-up sets a session `set_false_path -to [all_outputs]` so OpenROAD does not spend the budget on `resp_msg[*]` (course 20% I/O delay) instead of the OpenSTA register-to-register WNS. If TritonRoute cannot connect (DRT-0206), apply restores the source `6_final`. A legal size-up may still leave setup open; leftover is named. Never calls `signoff_all`. |
