...
| **LVS deep** | INTEGRATED | Filter unused CDL + FILL from DEF + well→VDD/VSS + FILL/TAP `blank_circuit` | `lvs_deep_*.json` transistor PASS · VIA flatten · DFF_X2 must-connect 2 · flatten-all before extract fails compare | `.lvs.ok` only on a real KLayout match |
